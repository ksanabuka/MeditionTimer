        ...
        VStack(alignment: .center) {
            Group {
                HStack() {
                    Spacer()
                    VStack {
                     Text(giveSessionPart(userMedSets: self.userMedSets, counter: self.counter))
                        

                     Text("Med_Name is \(self.userMedSets.med_name)")
                     Text("totalMeditationTime...: \(self.userMedSets.prep_time + self.userMedSets.med_time + self.userMedSets.rest_time)!")

                    }.foregroundColor(Color.white)
                }
    VStack {
      Text("\(countDownString(seconds: self.userMedSets.total_med_time))").onReceive(timer2){ _ in
         
         if (self.counter >= 0 && self.paused == false && self.userMedSets.prep_time + self.userMedSets.med_time + self.userMedSets.rest_time > 0 && self.counter < self.userMedSets.prep_time + self.userMedSets.med_time + self.userMedSets.rest_time)
         {
         if (self.counter == 0 && self.paused == false && self.userMedSets.prep_time + self.userMedSets.med_time + self.userMedSets.rest_time > 0)
         {
            print("beginning of non empty session at c: \(self.counter) \t cds: \(self.userMedSets.total_med_time)")
         }
         if (self.counter == 0 && self.userMedSets.prep_time != 0)
         {
            print(" beginning of Prep Time at c: \(self.counter) \t cds: \(self.userMedSets.total_med_time)")
            self.playChimeVibrate(chimeName: self.userMedSets.prep_chime, repeatPlay: self.userMedSets.repeat_prep_chime)
         }
                  
         if (self.counter == self.userMedSets.prep_time && self.userMedSets.med_time != 0) {
            self.myInervalCounter = 0
            print(" beginning med Time at c: \(self.counter) \t cds: \(self.userMedSets.total_med_time)")
            self.playChimeVibrate(chimeName: self.userMedSets.start_med_chime, repeatPlay: self.userMedSets.repeat_start_med_chime)

            }
           if inMeditationPart(userMedSets: self.userMedSets, counter: self.counter)
           {
            print("In med Session at c: \(self.counter) \t cds: \(self.userMedSets.total_med_time)")
            if (self.userMedSets.interval_time > 0) {
               if self.myInervalCounter == self.userMedSets.interval_time && self.counter < self.userMedSets.prep_time + self.userMedSets.med_time {
                  print("BUUM In med Session at c: \(self.counter) \t cds: \(self.userMedSets.total_med_time)")
                  self.playChimeVibrate(chimeName: self.userMedSets.interval_chime, repeatPlay: self.userMedSets.repeat_interval_chime)
                  self.myInervalCounter = 0
               }
               self.myInervalCounter += 1
            }}
            if (self.counter == self.userMedSets.prep_time + self.userMedSets.med_time && self.userMedSets.rest_time != 0) {
               print(" beginning Rest Time at c: \(self.counter) \t cds: \(self.userMedSets.total_med_time)")
               self.playChimeVibrate(chimeName: self.userMedSets.end_med_chime, repeatPlay: self.userMedSets.repeat_end_med_chime)

            }
            
            self.counter += 1
            self.userMedSets.total_med_time -= 1
         }
         
         else if (self.counter == self.userMedSets.prep_time + self.userMedSets.med_time + self.userMedSets.rest_time && self.paused == false && (self.userMedSets.rest_time != 0 || self.userMedSets.med_time != 0 || self.userMedSets.prep_time != 0))
         {
            print(" End of nonempty session at c: \(self.counter) \t cds: \(self.userMedSets.total_med_time)")
            self.playChimeVibrate(chimeName: self.userMedSets.end_rest_chime, repeatPlay: self.userMedSets.repeat_end_rest_chime)
            self.counter = 0
            self.myInervalCounter = 0
      ...
      self.userMedSets.total_med_time =         self.userMedSets.prep_time + self.userMedSets.med_time + self.userMedSets.rest_time
         ...
         self.medPart = giveSessionPart(userMedSets: self.userMedSets, counter: self.counter)
...
.onAppear {
   ...
   self.intervalCounter = (self.userMedSets.interval_time > 0 && self.userMedSets.interval_time < self.userMedSets.med_time) ? self.userMedSets.med_time / self.userMedSets.interval_time : 0
